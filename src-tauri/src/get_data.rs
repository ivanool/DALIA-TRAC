...
#[tauri::command]
pub fn get_emisora_info(emisora: String, trimestre: String) -> Result<String, String> {
    use crate::activos::{get_finantial_flow, get_finantial_position, get_quarterly_income_statement};
    use postgres::NoTls;
    use serde_json::json;
    ...
    // Financial Flow
    let cashflow = match get_finantial_flow(&mut client, &emisora, &trimestre) {
        Ok(data) => data,
        Err(e) => return Err(format!("Error getting financial flow: {}", e)),
    };
    // Financial Position
    let position = match get_finantial_position(&mut client, &emisora, &trimestre) {
        Ok(data) => data,
        Err(e) => return Err(format!("Error getting financial position: {}", e)),
    };
    // Income Statement
    let income = match get_quarterly_income_statement(&mut client, &emisora, &trimestre) {
        Ok(data) => data,
        Err(e) => return Err(format!("Error getting income statement: {}", e)),
    };
    // Intradia (stub: returns empty for now, or you can implement a real call)
    let intradia = serde_json::Value::Null;
    // Optionally, you could call get_intradia here and parse results if needed
